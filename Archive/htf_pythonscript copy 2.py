...
# - Cells `1-13` are the legacy `8h` pipeline that produced the original
#   `htf_backtest`, `htf_features`, `htf_4class_labels`, and `shift4h` outputs.
# - Cell `14` is the multi-regime extension and is the source of truth for
#   additional batch regimes such as `24h` and `7d`.
# - Legacy names like `period_8h_start` and `shift4h` are intentionally kept in
#   the older cells for backward compatibility. The multi-regime path keeps the
#   same column name as a compatibility alias and records the real regime in
#   `batch_regime`, `batch_duration_hours`, and `family_shift_hours`.
#
# ## Legacy 8h Data Preparation Summary
#
#
...
# - Cell `14` builds separate artifact trees for `24h` and `7d`
# - Family `B` is unshifted, family `C` is shifted by half the regime duration
# - Labels still use the same `1m` targets: `target_4class` and `target_breakfree`

# %%
# =============================================================================
# CELL 2: SETUP & PATHS (legacy 8h flow + shared config)
# =============================================================================
import gc
import os
import hashlib
...
set_run_stage(
    ...
    detail=f"regimes={MULTI_REGIME_BUILD_REGIMES}, helpers={MULTI_REGIME_RUN_HELPERS}, optimization={MULTI_REGIME_RUN_OPTIMIZATION}",
)
print("=" * 70)
...
if RUN_MULTI_REGIME_EXTENSION:
    multi_regime_config = MultiRegimeHTFConfig(
        project_root=PROJECT_ROOT,
        data_dir=PROJECT_ROOT / "data",
        raw_data_dir=PROJECT_ROOT / "fetchingByBit",
        pipeline_artifact_version=PIPELINE_ARTIFACT_VERSION,
        thresholds_by_tf=TF_THRESHOLDS,
        distance_windows_by_tf=DISTANCE_WINDOWS_BY_TF,
        ...
        breakfree_threshold_1m=BREAKFREE_THRESHOLD,
