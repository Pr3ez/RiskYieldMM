...
from regression_feature_engineering.features.regime_calendar_state import (
    RETIRED_FUTURE_DERIVED_FEATURE_SUFFIXES,
)
...
def resolve_context(
    *,
    ...
    feature_root = (
        data_root / "htf_multiasset" / asset_slug / FEATURE_SET / layout.root_id / "1m"
    )
    label_root = (
        data_root
        / "htf_multiasset"
...
def load_rpf_manifest(path: Path) -> RPFManifest:
    ...
    if diagnostic:
        raise ValueError(
            f"RPF manifest feature_columns include diagnostics: {diagnostic[:5]}"
        )
    retired_future_derived = [
        col
        for col in feature_columns
        if col.startswith("rpf_regime_")
        and col.endswith(RETIRED_FUTURE_DERIVED_FEATURE_SUFFIXES)
    ]
    if retired_future_derived:
        raise ValueError(
            "RPF manifest feature_columns include retired future-derived "
            f"observed-session features: {retired_future_derived[:5]}. "
            "Rebuild the feature root before model evaluation."
        )
    return RPFManifest(
        path=path,
...
def available_batch_ids(feature_root: Path, label_root: Path) -> tuple[int, ...]:
    feature_ids = {
        batch_id_from_path(path) for path in Path(feature_root).glob("batch_*.parquet")
    }
    label_ids = {
        batch_id_from_path(path) for path in Path(label_root).glob("batch_*.parquet")
    }
...
def build_batch_index(context: RPFDataContext) -> pl.DataFrame:
    ...
    label_paths = [
        context.label_root / f"batch_{batch_id:04d}.parquet" for batch_id in batch_ids
    ]
    feature_paths = [
        context.feature_root / f"batch_{batch_id:04d}.parquet" for batch_id in batch_ids
    ]
    labels = (
    ...
    features = pl.scan_parquet(feature_paths, extra_columns="ignore").select(
        ["timestamp", "batch_id"]
    )
...
def build_label_window_index(
    context: RPFDataContext, batch_ids: tuple[int, ...] | list[int] | None = None
) -> pl.DataFrame:
    batch_ids = tuple(
        int(value)
        for value in (
            batch_ids or available_batch_ids(context.feature_root, context.label_root)
        )
    )
    if not batch_ids:
    ...
    frame = (
        pl.scan_parquet(label_paths, extra_columns="ignore")
        .select(
            ["timestamp", "batch_id", VALID_COL, context.target_col, *LABEL_WINDOW_COLS]
        )
        .filter(
            pl.col(VALID_COL).fill_null(False)
            & pl.col(context.target_col).is_not_null()
        ...
        .agg(
            ...
            (pl.col("label_window_batch_id") != pl.col("batch_id"))
            .sum()
            .alias("cross_batch_label_count"),
            pl.col("target_reg_distance_horizon_minutes_v2")
            .min()
            .alias("horizon_minutes_min"),
            pl.col("target_reg_distance_horizon_minutes_v2")
            .max()
            .alias("horizon_minutes_max"),
        )
        .sort("batch_id")
        .select(
            [
...
def load_joined_batches(
    ...
    *,
    ...
    missing = [
        col for col in feature_columns if col not in context.manifest.feature_columns
    ]
    if missing:
        raise ValueError(
            f"Requested columns not present in RPF manifest: {missing[:5]}"
        )
    ...
    for batch_id in sorted({int(value) for value in batch_ids}):
        ...
        features = pl.read_parquet(
            feature_path, columns=["timestamp", "batch_id", *feature_columns]
        )
        labels = pl.read_parquet(
            label_path, columns=["timestamp", "batch_id", VALID_COL, context.target_col]
        )
        joined = (
...
def frame_to_numpy(
    ...
    *,
    ...
    model_frame = frame.select(["timestamp", "batch_id", target_col, *cols]).drop_nulls(
        [target_col, *cols]
    )
