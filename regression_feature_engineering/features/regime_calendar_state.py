...
context from future bars. In particular, observed-session end labels and
distances are not model inputs: without an authoritative point-in-time calendar,
the canonicalizer can only derive them from later rows in the stored dataset.
"""
...
RETIRED_FUTURE_DERIVED_FEATURE_SUFFIXES: tuple[str, ...] = (
    "_session_progress_bnd",
    "_minutes_to_close_bnd",
    "_session_close_bnd",
    "_weekly_close_bnd",
)
...
def enrich_regime_calendar_state_sources(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            f"Regime/calendar source bars missing columns: {sorted(missing)}"
        )
    ...
    out = _with_optional_metadata_defaults(bars.sort("timestamp")).with_columns(
        [
            ...
            (pl.col("high").cast(pl.Float64) - pl.col("low").cast(pl.Float64)).alias(
                "_rpf_regime_range"
            ),
        ]
    )
    out = out.with_columns(
        [
            safe_div_expr(pl.col("_rpf_regime_range"), pl.col("close")).alias(
                "_rpf_regime_range_pct"
            ),
            pct_change_expr(pl.col("close"), pl.col("close").shift(1)).alias(
                "_rpf_regime_ret_pct"
            ),
        ]
    )
    ...
    for lookback in lookbacks:
        ...
        out = out.with_columns(
            [
                ...
                pct_change_expr(pl.col("close"), pl.col("close").shift(lookback)).alias(
                    net_col
                ),
            ]
        )
        vol_rel_raw = safe_div_expr(
            pl.col("_rpf_regime_range_pct"), pl.col(range_median), default=0.0
        )
        trend_eff = bounded_expr(
            safe_div_expr(pl.col(net_col), pl.col(path_col), default=0.0),
            lower=-1.0,
            upper=1.0,
        )
        out = out.with_columns(
            [
                bounded_expr(
                    safe_div_expr(vol_rel_raw, 1.0 + vol_rel_raw, default=0.0),
                    lower=0.0,
                    upper=1.0,
                )
                .fill_null(0.0)
        ...
        out = out.with_columns(
            [
                bounded_expr(
                    (pl.col(trend_eff_col) + pl.col(trend_sign_col)) / 2.0,
                    lower=-1.0,
                    upper=1.0,
                )
                .fill_null(0.0)
...
def add_regime_calendar_state_features(
    ...
    *,
    ...
    minute_of_day = (pl.col("timestamp").dt.hour().cast(pl.Float64) * 60.0) + pl.col(
        "timestamp"
    ).dt.minute().cast(pl.Float64)
    day_of_week = (pl.col("timestamp").dt.weekday().cast(pl.Float64) - 1.0).clip(
        0.0, 6.0
    )
    out = df.with_columns(
        [
            ((minute_of_day / 1440.0) * (2.0 * math.pi))
            .sin()
            .alias("rpf_regime_utc_hour_sin"),
            ((minute_of_day / 1440.0) * (2.0 * math.pi))
            .cos()
            .alias("rpf_regime_utc_hour_cos"),
            ((day_of_week / 7.0) * (2.0 * math.pi))
            .sin()
            .alias("rpf_regime_utc_dow_sin"),
            ((day_of_week / 7.0) * (2.0 * math.pi))
            .cos()
            .alias("rpf_regime_utc_dow_cos"),
            (day_of_week >= 5.0)
            .cast(pl.Float64)
            .alias("rpf_regime_utc_is_weekend_bnd"),
        ]
    )
    for timeframe in timeframes:
        ...
        for suffix in (
            ...
            if source_col not in out.columns:
                raise ValueError(
                    f"Missing regime/calendar row source column: {source_col}"
                )
            out = out.with_columns(
                pl.col(source_col)
                .cast(pl.Float64)
...
def _with_optional_metadata_defaults(bars: pl.DataFrame) -> pl.DataFrame:
    ...
    additions = [
        expr.alias(name) for name, expr in defaults.items() if name not in bars.columns
    ]
