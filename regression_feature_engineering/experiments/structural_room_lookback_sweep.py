"""Sweep structural-room lookback sets without rewriting feature roots."""
...
from regression_feature_engineering.features.structural_room import (  # noqa: E402
    ...
    source_columns as structural_source_columns,
)
...
WIDTH_TARGETS = ("target_extreme_total", "target_mean_total")
INDIVIDUAL_TARGETS = TARGET_COLUMNS


@dataclass(frozen=True)
class LookbackSweepResult:
    """Summary for one structural-room lookback set."""
...
def run_structural_room_lookback_sweep(
    *,
    project_root: Path,
    asset_id: str,
    root_key: str,
    timeframes: tuple[str, ...] = DEFAULT_TIMEFRAMES,
    lookback_sets: tuple[tuple[int, ...], ...] = (DEFAULT_ROOM_LOOKBACKS,),
    output_dir: Path | None = None,
    batch_limit: int | None = None,
) -> list[LookbackSweepResult]:
    """Run a report-only structural-room lookback sweep."""
    ...
    output_dir = output_dir or project_root / "test_output/regression_feature_engineering_structural_room_sweeps"
    ...
    results: list[LookbackSweepResult] = []
...
def _run_one_set(
    *,
    ...
    enriched = {
        timeframe: enrich_structural_room_sources(bars, lookbacks=lookbacks)
        for timeframe, bars in bars_by_timeframe.items()
    }
    source_columns = tuple(dict.fromkeys((*OHLCV_COLUMNS, *structural_source_columns(lookbacks=lookbacks))))
    ...
    out = add_structural_room_features(aligned, timeframes=timeframes, lookbacks=lookbacks)
    feature_cols = structural_feature_columns(timeframes=timeframes, lookbacks=lookbacks)
    ...
    width_best = _best_row(correlations, WIDTH_TARGETS)
    individual_best = _best_row(correlations, INDIVIDUAL_TARGETS)
    result = LookbackSweepResult(
        asset_id=asset_id,
        root_key=root_key,
        root_id=root_id,
        lookbacks=lookbacks,
        ...
        max_abs_width_spearman=_row_float(width_best, "abs_spearman"),
        ...
        best_width_feature=_row_str(width_best, "feature"),
        best_width_target=_row_str(width_best, "target"),
    )
...
def _write_sweep_index(output_dir: Path, results: list[LookbackSweepResult]) -> None:
    rows = []
...
def _markdown_summary(rows: list[dict[str, Any]]) -> str:
    columns = [
        ...
        "max_abs_width_spearman",
        "best_width_feature",
        "best_width_target",
    ]
    return "# Structural Room Lookback Sweep\n\n" + _markdown_table(rows, columns) + "\n"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sweep structural-room lookback sets.")
    ...
    parser.add_argument("--lookback-sets", default="2,4,8;4,16,48;8,24,72;16,48,144")
    parser.add_argument(
        "--batch-limit",
        type=int,
        default=200,
    ...
    parser.add_argument("--output-dir", default="test_output/regression_feature_engineering_structural_room_sweeps")
    return parser.parse_args()


def main() -> int:
    args = _parse_args()
    results = run_structural_room_lookback_sweep(
    ...
    for result in results:
        print(
            ...
            f"direction={result.max_abs_direction_spearman} width={result.max_abs_width_spearman}"
        )
    return 0
