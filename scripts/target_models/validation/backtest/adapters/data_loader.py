...
def parse_config(config_name: str) -> dict[str, Any]:
    ...
    - volatility_regime: 2 (DECREASE/INCREASE binary)
...
def compute_common_valid_pred_idx(
    configs: list[str],
    common_range: dict | None = None,
) -> np.ndarray:
    """
    Compute the intersection of valid pred_idx values across all configs.

    This ensures all configs have exactly the same samples after NaN filtering,
    enabling synchronized walk-forward backtesting.

    Parameters
    ----------
    configs : list[str]
        List of config names to check
    common_range : dict or None
        Result from compute_common_pred_idx_range()

    Returns
    -------
    np.ndarray
        Array of pred_idx values that are valid (non-NaN target) in ALL configs
    """
    valid_pred_idx_sets = []
    ...
    for config_name in configs:
        ...
        # Get target series and find valid (non-NaN) rows
        _, target_series = get_optimal_target_for_config(combined_df, config_name)
        valid_mask = target_series.notna().values
        valid_pred_idx = set(pred_idx[valid_mask])
        valid_pred_idx_sets.append(valid_pred_idx)

    # Intersection of all valid pred_idx sets
    common_valid = set.intersection(*valid_pred_idx_sets)

    # Return sorted array
    return np.array(sorted(common_valid), dtype=np.int64)
...
def load_config_data(
    ...
    common_range: dict | None = None,
    common_valid_pred_idx: np.ndarray | None = None,
) -> ConfigData:
    ...
    # - volatility_regime_*: uses y_volatility_regime (binary: DECREASE/INCREASE)
    ...
    # Extract features (everything except pred_idx, timestamp, and target columns)
    feature_cols = [
        c
        for c in combined_df.columns
        if c not in ("pred_idx", "timestamp") and not c.startswith("y_")
    ]
    ...
    # If common_valid_pred_idx provided, use it (ensures all configs have same samples)
    # Otherwise fall back to per-config NaN filtering
    if common_valid_pred_idx is not None:
        # Use pre-computed common valid pred_idx (intersection across all configs)
        valid_mask = np.isin(pred_idx, common_valid_pred_idx)
    else:
        # Legacy behavior: filter out NaN in this config's target only
    ...
    # Get timestamps directly from combined dataset (authoritative source)
    # combined_datasets.py now includes timestamp column from assembled.parquet index
    if "timestamp" in combined_df.columns:
        all_timestamps = combined_df["timestamp"].values
        timestamps_valid = all_timestamps[valid_mask]
    else:
        # Fallback for old combined datasets without timestamp column
