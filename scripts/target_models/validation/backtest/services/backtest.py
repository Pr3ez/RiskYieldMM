...
def run_sync_backtest(
    ...
    if verbose:
        ...
        if cfg.enable_optuna:
            print(f"\nOptuna tuning: ENABLED ({cfg.optuna_timeout}s timeout)")
            print("  → Hyperparameters tuned fresh at each step")
            print("  → Warm-starting from previous step's best params")
            print("  → Per-model trials: CB=25, LGB=15, LSTM=10, Linear=20")
        else:
    ...
    # PHASE 1b: Compute common valid pred_idx (intersection of non-NaN samples)
    if verbose:
        print("Computing common valid samples across all configs...")

    common_valid_pred_idx = compute_common_valid_pred_idx(
        configs=configs_list, common_range=common_range
    )
    if verbose:
        print(
            f"  Common valid samples: {len(common_valid_pred_idx)} (after NaN filtering)"
        )

    # PHASE 2: Load all config data (aligned to common valid samples)
    if verbose:
    ...
    for i, config_name in enumerate(configs_list):
        try:
            data = load_config_data(
                ...
                common_valid_pred_idx=common_valid_pred_idx,
            )
    ...
    # Also handle spread_steps mode: evenly distribute steps across full range
    step_indices: list[int] | None = None  # Custom step positions (for spread mode)
    ...
    if cfg.n_steps is not None and cfg.n_steps > 0:
        ...
        elif cfg.spread_steps:
            # SPREAD MODE: Evenly distribute n_steps across full data range
            n_iterations = cfg.n_steps
            start_idx = effective_window  # Start from beginning
            # Calculate step positions: first, last, and evenly spaced in between
            step_indices = np.linspace(
                0, max_iterations - 1, n_iterations, dtype=int
            ).tolist()
            print(
                f"SPREAD MODE: {n_iterations} steps evenly distributed across {max_iterations} available"
            )
        else:
    ...
    if verbose:
        print(f"Embargo gap: {cfg.embargo_bars} bars between train/val/cal splits")
    ...
    for step in range(n_iterations):
        # Handle spread mode: use pre-computed step indices
        if step_indices is not None:
            pred_position = effective_window + step_indices[step] * cfg.step_size
        else:
            pred_position = start_idx + step * cfg.step_size

        # Print step START header (dynamic window optimization will print below this)
        if verbose:
            # Get step date if timestamps available
            first_data = next(iter(all_data.values()))
            if first_data.timestamps is not None and pred_position < len(
                first_data.timestamps
            ):
                step_date = first_data.timestamps[pred_position]
                step_date_str = f" | Date: {step_date}"
            else:
                step_date_str = ""
            print(f"\n{'#' * 80}")
            print(f"#  STEP {step + 1}/{n_iterations} START{step_date_str}")
            print(f"#  pred_position={pred_position}")
            print(f"{'#' * 80}")
        ...
        for _config_idx, (_config_name, data) in enumerate(all_data.items()):
            ...
            # Get timestamps for date printing (if available)
            if data.timestamps is not None:
                import pandas as pd

                timestamps_full = data.timestamps[window_start:window_end]
                pred_timestamp = data.timestamps[pred_position]
                # Target timestamp is horizon bars ahead - compute mathematically
                # (8h bars, so horizon * 8 hours)
                bar_duration = pd.Timedelta(hours=8)
                target_timestamp = pred_timestamp + (data.horizon * bar_duration)
            else:
...
def _display_step_metrics(
    ...
    if cls_configs:
        ...
        for config_name, data in cls_configs:
            ...
            # ----------------------------------------------------------------
            # NaN CHECK: HALT on NaN with full diagnostics
            # ----------------------------------------------------------------
            def _assert_no_nan(arr: np.ndarray, name: str, config: str) -> None:
                """Check for NaN and HALT with diagnostics if found."""
                ...
                if nan_mask.any():
                    ...
                    msg = (
                        f"\n{'=' * 70}\n"
                        f"FATAL: NaN in predictions - HALTING\n"
                        f"{'=' * 70}\n"
                        f"Config: {config}\n"
                        f"Array:  {name}\n"
                        f"Count:  {nan_count}/{len(arr)} ({100 * nan_count / len(arr):.1f}%)\n"
                        f"Indices: {nan_indices.tolist()}\n"
                        f"{'=' * 70}"
                    )
                    raise ValueError(msg)

            _assert_no_nan(y_true_arr, "y_true", config_name)
            _assert_no_nan(y_pred_arr, "y_pred", config_name)
            ...
            if "cb_pred" in preds.columns:
                ...
                _assert_no_nan(cb_preds, "cb_pred", config_name)
                _assert_no_nan(lgb_preds, "lgb_pred", config_name)
                _assert_no_nan(lstm_preds, "lstm_pred", config_name)
                _assert_no_nan(lin_preds, "linear_pred", config_name)
            else:
            ...
            if (
                ...
                try:
                    y_prob_arr = preds["y_prob"].values
                    if np.isnan(y_prob_arr).any():
                        raise ValueError("NaN in y_prob")
