#!/usr/bin/env python3
"""Independently verify the case-435 C3 exactness join certificate."""
...
RAW_AUTHORITY_SHA256 = {
    ...
    "tests/raw_v8_step2_inventory_v4_v49f.json": (
...
UPPER_DOMAIN = "RiskYieldMMStep2MaximumProtocolV2Case435UpperCertificateV1V4_9F_RawV8"
ATTAINER_DOMAIN = (
...
JOIN_MEMBERS = {
    ...
    "case_binding",
    "channel_source_sha256_by_path",
    "correction_subgate",
    "exact_maximum_octets",
    "exact_maximum_proved",
    "exactness_claimed",
    "exactness_join_version",
    "next_subgate",
    "ordered_required_join_predicates",
    "schedule_authority",
    "shared_raw_authority_sha256_by_path",
    "upper_channel",
...
class ExactnessError(RuntimeError):
    """Raised when a channel or join certificate fails closed."""


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise ExactnessError(message)


def _reject_number(value: str) -> Any:
    raise ExactnessError(f"non-integer JSON number is forbidden: {value}")
...
def _sealed_id(value: dict[str, Any], member: str, domain: str) -> str:
    return _sha256(
        _canonical_bytes(
            {
                "domain": domain,
                "payload": {key: item for key, item in value.items() if key != member},
            }
        )
    )


def _read_authorities(root: pathlib.Path) -> tuple[dict[str, Any], dict[str, Any]]:
    documents: dict[str, dict[str, Any]] = {}
    for relative, digest in {**RAW_AUTHORITY_SHA256, **CHANNEL_SOURCE_SHA256}.items():
        path = root / relative
        _require(path.is_file() and not path.is_symlink(), f"source absent: {relative}")
        raw = path.read_bytes()
        _require(_sha256(raw) == digest, f"source drift: {relative}")
        if relative in {SEED_PATH, REGISTRY_PATH}:
            documents[relative] = _strict_load(raw, relative)
    return documents[SEED_PATH], documents[REGISTRY_PATH]


def _expected_case() -> dict[str, Any]:
    return {
...
def _expected_schedule(seed: dict[str, Any]) -> dict[str, Any]:
    ...
    operation = program["application_schedule_operation"]
    instructions = operation["ordered_schedule_segment_records"][0][
        "ordered_application_instruction_records"
    ]
    return {
        "application_invocation_count": operation["application_invocation_count"],
        "application_schedule_operation_position": operation["operation_position"],
        "cross_rule_evaluation_count": operation["cross_rule_evaluation_count"],
        "direct_cross_expression_node_count": operation[
...
def verify(
    ...
    seed, registry = _read_authorities(root)
    ...
    _require(set(joined) == JOIN_MEMBERS, "join certificate members differ")
    _require(
        upper["case435_upper_certificate_id"]
        == _sealed_id(upper, "case435_upper_certificate_id", UPPER_DOMAIN)
        == UPPER_CERTIFICATE_ID,
        "upper certificate identity differs",
    )
    _require(
        attainer["case435_attainer_certificate_id"]
        == _sealed_id(attainer, "case435_attainer_certificate_id", ATTAINER_DOMAIN)
        == ATTAINER_CERTIFICATE_ID,
        "attainer certificate identity differs",
    )
    supplied_join_id = joined["case435_exactness_join_certificate_id"]
    _require(
        supplied_join_id
        == _sealed_id(joined, "case435_exactness_join_certificate_id", JOIN_DOMAIN),
        "join certificate identity differs",
    )
    expected_case = _expected_case()
    raw_authorities = dict(sorted(RAW_AUTHORITY_SHA256.items()))
    schedule = _expected_schedule(seed)
    _require(
        upper["case_binding"] == attainer["case_binding"] == expected_case
        and upper["authority_sha256_by_path"]
        == attainer["authority_sha256_by_path"]
        == raw_authorities
        and upper["case435_dependency_manifest_id"]
        == attainer["case435_dependency_manifest_id"]
        == DEPENDENCY_MANIFEST_ID,
        "channel problem identity join differs",
    )
    ...
    _require(
        ...
        and upper["canonical_composition"]["exact_upper_bound_octets"] == upper_octets,
    ...
    _require(
        ...
        and attainer["schedule_authority"] == schedule
    ...
    _require(
        ...
        and type(attainer_octets) is int
        and upper_octets > 0
        and upper_octets == attainer_octets,
    ...
    expected: dict[str, Any] = {
        ...
        "shared_raw_authority_sha256_by_path": raw_authorities,
    ...
    expected["case435_exactness_join_certificate_id"] = _sha256(
        _canonical_bytes({"domain": JOIN_DOMAIN, "payload": expected})
    )
    _require(joined == expected, "exactness join certificate projection differs")
    return {
        "case435_exactness_join_certificate_id": supplied_join_id,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(argv) not in {3, 4}:
        raise SystemExit(
            "usage: check...py [repository-root] upper.json attainer.json join.json"
        )
    root = pathlib.Path(argv[0]).resolve() if len(argv) == 4 else pathlib.Path.cwd()
    offset = 1 if len(argv) == 4 else 0
    try:
        upper = _strict_load(
        ...
        joined = _strict_load(
            pathlib.Path(argv[offset + 2]).read_bytes(), "join certificate"
        )
        report = verify(root, upper, attainer, joined)
    except (ExactnessError, OSError, KeyError, TypeError, ValueError) as error:
        sys.stderr.write(f"CASE435_EXACTNESS_CERTIFICATE_REJECT: {error}\n")
        return 1
