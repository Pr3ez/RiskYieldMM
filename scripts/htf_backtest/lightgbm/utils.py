...
class BaseOptimizerConfig:
    ...
    optuna_metric: str = (
        "accuracy"  # accuracy, log_loss, balanced_accuracy, macro_f1, macro_f1_up, macro_f1_down, directional_accuracy
    )

    def optuna_direction(self) -> str:
        """Optimization direction for the configured metric."""
        if self.optuna_metric in {"log_loss"}:
            return "minimize"
        return "maximize"
...
def load_batch(
    ...
    elif target_col == "target_8class" and "target_name" in df_labels.columns:
...
def get_valid_batches(
    ...
    common mask target (for example target_8class) for batch completeness checks
...
def get_feature_columns(
    df: pl.DataFrame, target_col: str = "target_8class"
) -> list[str]:
...
def compute_directional_accuracy(
    ...
    # Fallback if names are not directional
    if all(d is None for d in directions):
        # Binary: class 0=down, 1=up. Multiclass: first half down, second half up.
        if n_classes == 2:
            directions = [0, 1]
        else:
    ...
    dir_true = np.array([directions[int(c)] for c in y_true], dtype=np.int8)
    dir_pred = np.array([directions[int(c)] for c in y_pred], dtype=np.int8)
    if len(dir_true) == 0:
        return 0.0
