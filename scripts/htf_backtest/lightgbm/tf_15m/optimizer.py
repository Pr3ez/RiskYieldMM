"""
15-Minute Timeframe Optimizer for HTF LightGBM Backtest
=======================================================

Self-contained 15m optimization with:
- 15m-specific configuration
- 15m-specific search spaces
...
# ============================================================================
@dataclass
class Config15m(BaseOptimizerConfig):
    """
    15m-specific configuration.

    15m has:
    - 32 bars per 8h batch (vs 96 for 5m)
    - Less noise, cleaner signals
    - Fewer samples per batch, needs larger windows
    """

    # Fewer trials for 15m (cleaner data)
    optuna_trials: int = 20
    optuna_timeout: int = 120


@dataclass
class WindowSpace15m(WindowSearchSpace):
    """
    15m window search space.

    15m has 32 rows per batch, so needs more batches for same samples.
    """

    lookback_min: int = 150  # ~4,800 rows minimum
    lookback_max: int = 500  # ~16,000 rows maximum

    # 15m less noisy, can weight older data more
...
@dataclass
class FeatureSpace15m(FeatureSearchSpace):
    """
    15m feature search space.

    15m has cleaner features, can use more of them.
    """

    feature_k_min: int = 20
    feature_k_max: int = 100

    # 15m features less correlated
...
@dataclass
class ModelSpace15m(ModelSearchSpace):
    """
    15m model search space.

    15m can use less regularization due to cleaner data.
    """

    # Standard learning rate for cleaner data
...
# 15M STEP OPTIMIZER
# ============================================================================
class StepOptimizer15m:
    """
    Optimizer for a single walk-forward step (15m timeframe).
    ...
    def __init__(
        self,
        config: Config15m,
        window_space: WindowSpace15m,
        feature_space: FeatureSpace15m,
        model_space: ModelSpace15m,
    ):
        self.config = config
    ...
    def optimize(
        ...
        if study_name is None:
            study_name = f"htf_15m_train_end_{train_end}"
...
class Optimizer15m:
    ...
    Wraps StepOptimizer15m with 15m-specific search spaces.
    """
    ...
    def create_step_optimizer(self) -> StepOptimizer15m:
        """Create StepOptimizer15m with 15m search spaces."""
