...
import scripts.workflow.config as wf_config_mod
...
# Regime-focused profile for this notebook copy:
# keep only non-redundant targets needed for 8h trend/regime work.
TARGET_PROFILE = "regime_core"
TARGET_PROFILES = {
    "regime_core": ["direction", "volatility_regime", "trend_regime"],
    "full": list(WORKFLOW_TARGETS),
}
if TARGET_PROFILE not in TARGET_PROFILES:
    raise ValueError(f"Unknown TARGET_PROFILE={TARGET_PROFILE}")

WORKFLOW_TARGETS = list(TARGET_PROFILES[TARGET_PROFILE])
WORKFLOW_HORIZONS = [1]
L1_CONFIG_MODE = "1bar"

# Patch workflow module globals for consistency across imported helpers in this process.
wf_config_mod.WORKFLOW_TARGETS = list(WORKFLOW_TARGETS)
wf_config_mod.WORKFLOW_HORIZONS = list(WORKFLOW_HORIZONS)
wf_config_mod.L1_CONFIG_MODE = L1_CONFIG_MODE

print(f"Active target profile: {TARGET_PROFILE}")
print(f"Active targets: {WORKFLOW_TARGETS}")
print(f"Active horizons: {WORKFLOW_HORIZONS}")
...
def validate_step_output(
    *,
    ...
    """Validate parquet output integrity for incremental workflow runs."""
    if not path.exists():
        raise FileNotFoundError(f"{step_name}: missing output file: {path}")

    df = pl.read_parquet(path)
    rows = len(df)
    if expected_rows is not None and rows != expected_rows:
        raise RuntimeError(
            f"{step_name}: row mismatch for {path.name}: {rows} != {expected_rows}"
        )

    if ts_col not in df.columns:
        raise RuntimeError(
            f"{step_name}: missing timestamp column '{ts_col}' in {path}"
        )

    ts = df[ts_col]
    if not ts.is_sorted():
        raise RuntimeError(f"{step_name}: timestamp is not sorted in {path.name}")

    dup_count = int(ts.is_duplicated().sum())
    if dup_count > 0:
        raise RuntimeError(
            f"{step_name}: found {dup_count} duplicate timestamps in {path.name}"
        )
    ...
    print(
        f"✓ {step_name} validated: rows={out['rows']}, ts=[{out['ts_min']} .. {out['ts_max']}]"
    )
    return out
...
# ----------------------------------------------------------------------------
from scripts.workflow.data_fetching import (
    get_raw_8h_info,
...
from scripts.workflow.state_detection import detect_new_data, get_data_status
...
raw_info = get_raw_8h_info(PROJECT_ROOT)
current_raw_last_ts = raw_info.get("last_timestamp")
status_snapshot = get_data_status(PROJECT_ROOT)
print(
    "Data status snapshot: "
    f"raw={status_snapshot.raw_rows}, merged={status_snapshot.merged_rows}, "
    f"features={status_snapshot.features_rows}, analysis={status_snapshot.analysis_rows}, "
    f"new_data_detected={status_snapshot.new_data_detected}, reason={status_snapshot.sync_reason}"
)
...
if not NEW_DATA_DETECTED and merged_file.exists():
    ...
    merged_last_ts = (
        str(merged_df.select("RAW_TM_timestamp").max().item())
        if "RAW_TM_timestamp" in merged_df.columns
        else None
    )
    if len(merged_df) == current_raw_rows and merged_last_ts == current_raw_last_ts:
...
validate_step_output(
    step_name="Step 1a merge",
    path=merged_file,
    ts_col="RAW_TM_timestamp",
    expected_rows=current_raw_rows,
)
...
if not NEW_DATA_DETECTED and features_file.exists():
    ...
    features_last_ts = (
        str(features_df_check.select("timestamp").max().item())
        if "timestamp" in features_df_check.columns
        else None
    )
    if (
        len(features_df_check) == current_raw_rows
        and features_last_ts == current_raw_last_ts
    ):
...
if not NEW_DATA_DETECTED and analysis_file.exists():
    ...
    analysis_last_ts = (
        str(analysis_check.select("timestamp").max().item())
        if "timestamp" in analysis_check.columns
        else None
    )
    if (
        len(analysis_check) == current_raw_rows
        and analysis_last_ts == current_raw_last_ts
    ):
