...
def _resolve_stage1_triplet_grid():
    ...
    try:
        if STAGE1_PORTFOLIO_REBUILD or not autogen_path.exists() or not portfolio_path.exists():
    ...
    for unit, manual_triplets in STAGE1_FALLBACK_MANUAL_TRIPLETS.items():
        ...
        report[unit] = {
            "source": (
                str(u.get("selected_source", "auto"))
                if validated
                else "manual"
            ),
            "selected_triplets": [list(t) for t in (validated if validated else manual_triplets)],
...
STAGE1_TRIPLET_GRID_BY_UNIT, STAGE1_PORTFOLIO_REPORT_BY_UNIT, STAGE1_PORTFOLIO_BUILD_RESULT = (
    _resolve_stage1_triplet_grid()
)


def _stage1_tf_spec(tf: str, lookback_max: int, min_samples_4: int, min_samples_breakfree: int):
    triplet_grid_4 = list(STAGE1_TRIPLET_GRID_BY_UNIT.get(f"{tf}/target_4class", []))
    triplet_grid_breakfree = list(STAGE1_TRIPLET_GRID_BY_UNIT.get(f"{tf}/target_breakfree", []))
...
STAGE1_MODEL_BACKTEST_SPECS = merge_backtest_specs(
    {
        "catboost": {
            "timeframes": {
                "1m": _stage1_tf_spec("1m", lookback_max=450, min_samples_4=150, min_samples_breakfree=150),
                "5m": _stage1_tf_spec("5m", lookback_max=450, min_samples_4=100, min_samples_breakfree=100),
                "15m": _stage1_tf_spec("15m", lookback_max=450, min_samples_4=50, min_samples_breakfree=50),
            }
        }
    },
    {},
...
def _recover_stage1_maps_from_artifacts(
    ...
    for tf_dir in sorted([p for p in base_dir.iterdir() if p.is_dir()]):
        ...
        for target_dir in sorted([p for p in tf_dir.iterdir() if p.is_dir()]):
            snapshots = sorted(target_dir.glob("batch_*/stage1/stage1_config_snapshot.json"))
            if not snapshots:
                continue
            ...
            n_classes = int(cfg.get("n_classes", max(2, len(cfg.get("class_names", [])))))
            class_names = list(cfg.get("class_names", [f"class_{i}" for i in range(n_classes)]))
...
if STAGE1_STEP2_MAX_STEPS_PER_UNIT is None or int(STAGE1_STEP2_MAX_STEPS_PER_UNIT) >= 500:
