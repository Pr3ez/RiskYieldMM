...
# WORKFLOW_TARGETS: Which targets to process (returns removed - redundant)
WORKFLOW_HORIZONS = [1]  # DEFAULT: Only 1-bar (fastest)
WORKFLOW_TARGETS = ["direction", "volatility", "vol_regime", "trend_regime"]

# AUTO_FETCH: Automatically fetch & aggregate new data if stale
# Set to False only if you want to skip data updates entirely
AUTO_FETCH_IF_STALE = True
STALE_THRESHOLD_HOURS = 16  # 2 bars behind = stale

print("\n⚙️ Workflow Config:")
print(f"   Horizons: {WORKFLOW_HORIZONS}")
print(f"   Targets: {WORKFLOW_TARGETS}")
print(f"   Total configs: {len(WORKFLOW_HORIZONS) * len(WORKFLOW_TARGETS)}")
print(
    f"   Auto-fetch if stale: {AUTO_FETCH_IF_STALE} (threshold: {STALE_THRESHOLD_HOURS}h)"
)
...
# ============================================================================
# Fetch latest data from Bybit API and aggregate 4h → 8h
# This step runs AUTOMATICALLY if data is stale (>16h behind)
...
# %%
def run_step0_fetch_and_aggregate() -> bool:
    """
    Step 0: Fetch & aggregate data from Bybit.

    Returns True if data is fresh (either already current or successfully updated).
    """
    import polars as pl
    ...
    if OHLCV_8H.exists():
        ...
        is_stale = hours_behind > STALE_THRESHOLD_HOURS
    else:
    ...
    if not AUTO_FETCH_IF_STALE:
        print("\n⚠️ Data is STALE but AUTO_FETCH_IF_STALE=False")
...
#
# CRITICAL INSIGHT: Expanding rank optimization is NOT incrementally resumable!
# When new rows are added, ranks for ALL historical rows change because the
# distribution changes. Therefore:
# - Steps 0-2 (fetch, merge, features, targets): CAN be incremental
# - Step 3 (optimization): MUST recompute from scratch when source data changes
...
# Check if source data changed (triggers full recompute of downstream steps)
import polars as pl
...
if raw_8h_file.exists():
    ...
    current_raw_rows = len(raw_df)
    current_raw_ts = str(raw_df.select("timestamp").max().item())

    prev_state = pipeline_state.get_step("raw_8h")

    # Also check downstream files for staleness
    merged_8h_file = PROJECT_ROOT / "data" / "merged_8h_raw.parquet"
    downstream_stale = False
    if merged_8h_file.exists():
        downstream_rows = len(pl.read_parquet(merged_8h_file))
        if downstream_rows != current_raw_rows:
            downstream_stale = True
    else:
        downstream_stale = True  # No file = need to compute

    # Detect new data: state changed OR downstream files are stale
    if (
        prev_state.row_count and prev_state.row_count != current_raw_rows
    ) or downstream_stale:
        NEW_DATA_DETECTED = True
        if prev_state.row_count:
            new_rows = current_raw_rows - prev_state.row_count
            print(f"\n🔄 NEW DATA DETECTED: {new_rows} new 8h bars")
        else:
            print("\n🔄 DATA SYNC REQUIRED: Downstream files stale or missing")
    ...
    pipeline_state.update_step(
        "raw_8h",
        row_count=current_raw_rows,
        last_timestamp=current_raw_ts,
    )
else:
    NEW_DATA_DETECTED = True
    print("\n⚠️ Raw 8h data not found - full compute required")
...
if not NEW_DATA_DETECTED:
    # Check if merged file exists and is current
    merged_file = PROJECT_ROOT / "data" / "merged_8h_raw.parquet"
    if merged_file.exists():
        merged_df = pl.read_parquet(merged_file)
        merged_rows = len(merged_df)
        if merged_rows == current_raw_rows:
            print(f"✓ merged_8h_raw.parquet is current ({merged_rows} rows)")
            print("  Skipping merge (no new data)")
            df_raw = merged_df.to_pandas()
        else:
            NEW_DATA_DETECTED = True  # Force recompute
            print(f"⚠️ Row count mismatch: merged={merged_rows}, raw={current_raw_rows}")

if NEW_DATA_DETECTED or not (PROJECT_ROOT / "data" / "merged_8h_raw.parquet").exists():
    ...
    pipeline_state.update_step(
        ...
        last_timestamp=str(df_raw["timestamp"].max()),
...
if not NEW_DATA_DETECTED and features_file.exists():
    ...
    features_rows = len(features_df_check)
    if features_rows == current_raw_rows:
        print(f"✓ features_8h.parquet is current ({features_rows} rows)")
        print("  Skipping feature computation (no new data)")
        SKIP_FEATURES = True
        features_df = features_df_check.to_pandas()
    else:
        print(f"⚠️ Row count mismatch: features={features_rows}, raw={current_raw_rows}")
...
# Verify fracdiff features (AFML Ch.5)
...
if fracdiff_cols:
    ...
    for col in fracdiff_cols:
        ...
        print(
            f"  {col}: valid={valid:,} ({100 - nan_pct:.1f}%), range=[{features[col].min():.2f}, {features[col].max():.2f}]"
        )
else:
    print("\n⚠ No fracdiff features found (run Step 1b to generate)")
...
if not NEW_DATA_DETECTED and analysis_file.exists():
    ...
    analysis_rows = len(analysis_check)
    if analysis_rows == current_raw_rows:
        print(f"✓ analysis_8h.parquet is current ({analysis_rows} rows)")
        print("  Skipping target generation (no new data)")
        SKIP_ANALYSIS = True
        df_analysis = analysis_check
    else:
        print(f"⚠️ Row count mismatch: analysis={analysis_rows}, raw={current_raw_rows}")
...
if "y_tb_direction" in df_check.columns:
    ...
    print(f"  Valid labels: {len(tb_valid):,} ({100 * len(tb_valid) / total:.1f}%)")
    ...
    print("\n  Label distribution:")
    print(
        f"    y_tb_direction=1 (profitable): {label_1:,} ({100 * label_1 / len(tb_valid):.1f}%)"
    )
    print(
        f"    y_tb_direction=0 (unprofitable): {label_0:,} ({100 * label_0 / len(tb_valid):.1f}%)"
    )

    # Barrier distribution
    if "y_tb_barrier" in df_check.columns:
        print("\n  Barrier hit distribution:")
        for barrier in ["TP", "SL", "TIME"]:
            count = (df_check["y_tb_barrier"] == barrier).sum()
            pct = 100 * count / len(tb_valid)
            print(f"    {barrier}: {count:,} ({pct:.1f}%)")

    # Correlation with y_direction
    if "y_direction" in df_check.columns:
        # Filter to rows where both are valid
        both_valid = df_check.filter(
            pl.col("y_tb_direction").is_not_null() & pl.col("y_direction").is_not_null()
        )
        if len(both_valid) > 100:
            agreement = (
                both_valid["y_tb_direction"] == both_valid["y_direction"]
            ).mean()
            print(f"\n  Agreement with y_direction: {agreement:.1%}")
...
#
# **⚠️ CRITICAL: EXPANDING RANK IS NOT INCREMENTALLY RESUMABLE!**
#
# When new rows are added, ranks for ALL historical rows change because
# the distribution changes. Therefore this step MUST recompute from scratch
# whenever source data (analysis_8h.parquet) has new rows.
#
...
# - RollingZScore HELPS direction, HURTS volatility/regime targets
#
...
#
# ⚠️ RESUMABILITY: This step ALWAYS recomputes when source data changes!
# Expanding rank uses all historical data, so new rows change ALL ranks.
...
if not NEW_DATA_DETECTED:
    # Check if all optimized files exist with correct row counts
    all_opt_exist = True
    for target in WORKFLOW_TARGETS:
        for horizon in WORKFLOW_HORIZONS:
            ...
            if len(opt_df) != current_raw_rows:
                print(
                    f"⚠️ {opt_file.name}: row mismatch ({len(opt_df)} vs {current_raw_rows})"
                )
                all_opt_exist = False
                break
        if not all_opt_exist:
            break
...
if not SKIP_OPTIMIZATION:
    if NEW_DATA_DETECTED:
        print("\n⚠️ NEW DATA DETECTED - Full optimization required")
        print("   (Expanding rank uses all history, so new rows change ALL ranks)")
    ...
    # Update state for each optimized file
    for target in WORKFLOW_TARGETS:
        for horizon in WORKFLOW_HORIZONS:
            opt_file = (
                PROJECT_ROOT
...
#
# **Target types:**
# - `direction`: 3-class (DOWN/UP/NEUTRAL) — entry signals
# - `volatility`: |return| — risk/position sizing
# - `vol_regime`: LOW/MED/HIGH — regime-aware strategies
# - `trend_regime`: Up/Down trend — trend-following filter
#
# NOTE: 'returns' removed - redundant with direction + volatility
#
# **Horizons (configured via WORKFLOW_HORIZONS):**
# - `1bar` (8h): Intraday (DEFAULT)
...
# %%
# Step 4: Build the dataset matrix
from scripts.analysis.run import cmd_build_datasets

# Build datasets for selected targets and horizons
# Each dataset:
...
print(f"Building {expected_datasets}-dataset matrix (horizons={WORKFLOW_HORIZONS})...")
...
if ohlcv_8h.exists():
    ...
    checks.append((f"Step 0: 8h data ({hours_behind:.0f}h behind)", hours_behind <= 16))
else:
...
#
# **Config Modes (set L1_CONFIG_MODE below):**
# - `"1bar"`: 4 configs (direction, volatility, vol_regime, trend_regime) - **DEFAULT, fastest**
...
# %%
# Step 8: Precompute L1 helper features
#
...
# L1 PRECOMPUTE MODE:
# - "1bar": Only 4 configs (direction, volatility, vol_regime, trend_regime) - FASTEST
# - "reduced": 8 configs (1bar + 3bar horizons)
# - "all": All 16 configs (all 4 horizons)
# NOTE: 'returns' removed - redundant with direction + volatility
L1_CONFIG_MODE = "1bar"  # <-- CHANGE THIS to run more configs
...
# Use same mode as precomputation (or check all)
config_names = get_config_names(L1_CONFIG_MODE)
...
if missing_configs:
    ...
    print(
        f"\n✓ All {len(config_names)} configs ({L1_CONFIG_MODE} mode) precomputed and ready for Step 9: Dataset Assembly"
    )
...
#
# **What it does:**
# - For each of 16 configs, reads all iteration parquet files
