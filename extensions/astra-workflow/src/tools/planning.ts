...
export function createSetGoalTool(ctx: ToolContext): vscode.LanguageModelTool<SetGoalInput> {
  return {
    async invoke(
      options: vscode.LanguageModelToolInvocationOptions<SetGoalInput>,
      ...
      try {
        ...
        const validPhases = [WorkflowPhase.VALUE_GATE, WorkflowPhase.GOAL_DEFINITION, WorkflowPhase.IDLE];
        if (!validPhases.includes(state.phase)) {
          return createErrorResult(`Cannot set goal in phase: ${state.phase}. Complete current phase before setting goal.`);
        }

        // Assess goal quality
        let goalQuality: SetGoalOutput['goalQuality'] = 'clear';
        ...
        return createErrorResult(`Set goal failed: ${error}`);
      }
    }
  };
}
...
export function createSetContextTool(ctx: ToolContext): vscode.LanguageModelTool<SetContextInput> {
  return {
    async invoke(
      ...
      try {
        ...
        const validPhases = [WorkflowPhase.GOAL_DEFINITION, WorkflowPhase.CONTEXT_COLLECTION];
        if (!validPhases.includes(state.phase)) {
          return createErrorResult(`Cannot set context in phase: ${state.phase}. Set goal first. Use astra_set_goal before collecting context.`);
        }

        // Assess context completeness
        let completeness: SetContextOutput['completeness'] = 'adequate';
        ...
        return createErrorResult(`Set context failed: ${error}`);
      }
    }
  };
}
...
export function createSetPlanTool(ctx: ToolContext): vscode.LanguageModelTool<SetPlanInput> {
  return {
    async invoke(
      options: vscode.LanguageModelToolInvocationOptions<SetPlanInput>,
      _token: vscode.CancellationToken
    ): Promise<vscode.LanguageModelToolResult> {
      ...
      try {
        ...
        const validPhases = [WorkflowPhase.CONTEXT_COLLECTION, WorkflowPhase.PLANNING];
        if (!validPhases.includes(state.phase)) {
          return createErrorResult(`Cannot set plan in phase: ${state.phase}. Collect context first. Use astra_set_context before creating plan.`);
        }

        // Validate steps
        if (!input.steps || input.steps.length === 0) {
          return createErrorResult('Plan must have at least one step');
        }

        // Assess plan quality
        let planQuality: SetPlanOutput['planQuality'] = 'solid';
        let estimatedTotalMinutes = 0;

        // Convert input steps to workflow steps
        const workflowSteps: WorkflowStep[] = input.steps.map((step, index) => {
        ...
        return createErrorResult(`Set plan failed: ${error}`);
      }
    }
  };
}
