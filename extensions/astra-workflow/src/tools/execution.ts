...
export function createStepStartTool(ctx: ToolContext): vscode.LanguageModelTool<StepStartInput> {
  return {
    async invoke(
      options: vscode.LanguageModelToolInvocationOptions<StepStartInput>,
      ...
      try {
        ...
        const validPhases = [WorkflowPhase.PLANNING, WorkflowPhase.EXECUTING, WorkflowPhase.VALIDATING];
        if (!validPhases.includes(state.phase)) {
          return createErrorResult(`Cannot start step in phase: ${state.phase}. Create a plan first using astra_set_plan.`);
        }
        ...
        if (!step) {
          ...
          return createErrorResult(`Step not found: ${input.stepId}. Valid IDs: ${validIds}`);
        }
        ...
        if (currentInProgress && currentInProgress.id !== input.stepId) {
          return createErrorResult(`Another step is in progress: ${currentInProgress.id}. Complete or block current step before starting new one.`);
        }

        // Check if step is already completed
        if (step.status === 'completed') {
          return createErrorResult(`Step ${input.stepId} is already completed. Start a different step.`);
        }
        ...
        return createErrorResult(`Step start failed: ${error}`);
      }
    }
  };
}
...
export function createStepCompleteTool(ctx: ToolContext): vscode.LanguageModelTool<StepCompleteInput> {
  return {
    async invoke(
      options: vscode.LanguageModelToolInvocationOptions<StepCompleteInput>,
      _token: vscode.CancellationToken
    ): Promise<vscode.LanguageModelToolResult> {
      ...
      try {
        ...
        const step = state.steps.find(s => s.id === input.stepId);
        if (!step) {
          return createErrorResult(`Step not found: ${input.stepId}`);
        }

        // Check if step is in progress
        if (step.status !== 'in-progress') {
          return createErrorResult(`Step ${input.stepId} is not in progress (status: ${step.status}). Start the step first with astra_step_start.`);
        }

        // Validation check
        if (!input.validated) {
          return createErrorResult('Step must be validated before completion. Use astra_validate first, then complete with validated: true.');
        }
        ...
        let durationVsEstimate: StepCompleteOutput['durationVsEstimate'] = 'on-track';
        ...
        const output: StepCompleteOutput = {
          stepCompleted: true,
          durationVsEstimate,
        ...
        return createErrorResult(`Step complete failed: ${error}`);
      }
    }
  };
}
...
export function createStepBlockTool(ctx: ToolContext): vscode.LanguageModelTool<StepBlockInput> {
  return {
    async invoke(
      options: vscode.LanguageModelToolInvocationOptions<StepBlockInput>,
      _token: vscode.CancellationToken
    ): Promise<vscode.LanguageModelToolResult> {
      ...
      try {
        ...
        return createErrorResult(`Step block failed: ${error}`);
      }
    }
  };
}
...
export function createValidateTool(ctx: ToolContext): vscode.LanguageModelTool<ValidateInput> {
  return {
    async invoke(
      ...
      try {
        ...
        let validationQuality: ValidateOutput['validationQuality'] = 'adequate';
        ...
        return createErrorResult(`Validate failed: ${error}`);
      }
    }
  };
}
