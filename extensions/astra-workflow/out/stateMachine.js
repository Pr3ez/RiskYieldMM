...
exports.WorkflowStateMachine = void 0;
const types_js_1 = require("./types.js");
// =============================================================================
// DEFAULT VIRTUE METRICS
// =============================================================================
function createDefaultVirtueScore() {
    return {
...
function createDefaultVirtueMetrics() {
    return {
...
class WorkflowStateMachine {
    ...
    virtueMetrics;
    listeners = [];
    constructor(sessionId, initialState, virtueMetrics) {
        this.state = initialState ?? (0, types_js_1.createInitialState)(sessionId);
    ...
    getStrictness() {
        return types_js_1.STRICTNESS_BY_TYPE[this.state.taskType];
    }
    // ---------------------------------------------------------------------------
     ...
     */
    runVirtueGate(targetPhase, context) {
        const filters = [
     ...
     */
    checkWisdom(targetPhase, context) {
        const issues = [];
        let score = 100;
        // Check: Moving to execution without context?
        if (targetPhase === types_js_1.WorkflowPhase.EXECUTING && !this.state.context) {
        ...
        if (targetPhase === types_js_1.WorkflowPhase.PLANNING && !this.state.goal) {
     ...
     */
    checkTemperance(targetPhase, context) {
        const issues = [];
        let score = 100;
        // Check: Plan too complex for task type?
        if (this.state.taskType === types_js_1.TaskType.TRIVIAL && this.state.steps.length > 3) {
            issues.push('Trivial task has too many steps - over-engineering?');
            score -= 30;
        }
        if (this.state.taskType === types_js_1.TaskType.SIMPLE && this.state.steps.length > 5) {
     ...
     */
    checkCourage(targetPhase, context) {
        const issues = [];
        let score = 100;
        ...
        const unvalidatedCompleted = this.state.steps.filter(s => s.status === 'completed' && !s.validationResult).length;
     ...
     */
    checkJustice(targetPhase, context) {
        const issues = [];
        let score = 100;
    ...
    detectPathologies() {
        ...
        if (this.state.phase === types_js_1.WorkflowPhase.PLANNING &&
            this.state.context &&
        ...
        if (this.state.steps.length > 10 && this.state.taskType !== types_js_1.TaskType.COMPLEX) {
            pathologies.push({
        ...
        if (this.state.steps.length > 3 && completedCount === 0 && this.state.phase === types_js_1.WorkflowPhase.EXECUTING) {
            pathologies.push({
     ...
     */
    transition(toPhase, reason, useVirtueGate = true) {
        const fromPhase = this.state.phase;
        const validTargets = types_js_1.VALID_TRANSITIONS[fromPhase];
        // Validate transition
     ...
     */
    checkStrictnessViolation(toPhase, strictness) {
        const fromPhase = this.state.phase;
        // Check required phases aren't skipped
        if (toPhase === types_js_1.WorkflowPhase.PLANNING) {
        ...
        if (toPhase === types_js_1.WorkflowPhase.EXECUTING) {
            if (strictness.requireTodos && this.state.steps.length === 0) {
                return "Cannot proceed to execution: TODOs required but none created";
            }
        }
        if (toPhase === types_js_1.WorkflowPhase.COMPLETING) {
            if (strictness.requireValidation &&
                this.state.steps.some((s) => s.status === "completed" && !s.validationResult)) {
                return "Cannot complete: Some steps completed without validation";
            }
        }
        // Skip VALUE_GATE → GOAL_DEFINITION check for TRIVIAL tasks
        if (fromPhase === types_js_1.WorkflowPhase.IDLE &&
            toPhase === types_js_1.WorkflowPhase.GOAL_DEFINITION &&
     ...
     */
    startTask(taskType) {
        if (this.state.phase !== types_js_1.WorkflowPhase.IDLE) {
            return {
                success: false,
        ...
        const strictness = types_js_1.STRICTNESS_BY_TYPE[taskType];
     ...
     */
    passValueGate(valueJustification) {
        if (this.state.phase !== types_js_1.WorkflowPhase.VALUE_GATE) {
            return {
                success: false,
    ...
    failValueGate(reason) {
        ...
        return this.transition(types_js_1.WorkflowPhase.IDLE, `Value gate failed: ${reason}`);
    }
    /**
     * Set goal (GOAL_DEFINITION → CONTEXT_COLLECTION)
     */
    setGoal(goal) {
        if (this.state.phase !== types_js_1.WorkflowPhase.GOAL_DEFINITION) {
            return {
                success: false,
        ...
        return this.transition(types_js_1.WorkflowPhase.CONTEXT_COLLECTION, `Goal set: ${goal.statement}`);
    }
    /**
     * Set context (CONTEXT_COLLECTION → ENVIRONMENT_PRIMING or PLANNING)
     */
    setContext(context) {
        if (this.state.phase !== types_js_1.WorkflowPhase.CONTEXT_COLLECTION) {
            return {
                success: false,
        ...
        const targetPhase = context.dependencies.length > 0
    ...
    completeEnvironmentPriming() {
        if (this.state.phase !== types_js_1.WorkflowPhase.ENVIRONMENT_PRIMING) {
            return {
                success: false,
                error: `Cannot complete environment priming: Current phase is ${this.state.phase}`,
            };
        }
        return this.transition(types_js_1.WorkflowPhase.PLANNING, "Environment primed and ready");
    }
    /**
    ...
    setPlan(steps) {
        ...
        return this.transition(types_js_1.WorkflowPhase.EXECUTING, `Plan created with ${steps.length} steps`);
    }
    /**
     * Start executing a step
     */
    startStep(stepId) {
        if (this.state.phase !== types_js_1.WorkflowPhase.EXECUTING) {
            return {
                success: false,
     ...
     */
    completeStep(stepId, validationResult) {
        if (this.state.phase !== types_js_1.WorkflowPhase.EXECUTING) {
            return {
                success: false,
        ...
        const allComplete = this.state.steps.every((s) => s.status === "completed" || s.status === "skipped");
        if (allComplete) {
            return this.transition(types_js_1.WorkflowPhase.COMPLETING, "All steps completed");
        }
        return { success: true };
    }
    /**
    ...
    runDriftCheck() {
        ...
        for (const step of completedSteps) {
            if (step.estimatedMinutes &&
                ...
                issues.push(`Step "${step.title}" took ${step.actualMinutes}min vs estimated ${step.estimatedMinutes}min (>2x)`);
            }
        }
        // Reset counter
    ...
    blockStep(stepId, reason) {
        ...
        if (this.state.phase === types_js_1.WorkflowPhase.EXECUTING) {
            ...
            return this.transition(types_js_1.WorkflowPhase.BLOCKED, `Step blocked: ${reason}`);
        }
        return { success: true };
    }
    /**
     ...
     */
    complete(summary) {
        if (this.state.phase !== types_js_1.WorkflowPhase.COMPLETING) {
            return {
                success: false,
        ...
        return this.transition(types_js_1.WorkflowPhase.IDLE, `Completed: ${summary}`);
    }
    /**
     * Enter blocked state
     */
    block(reason) {
        ...
        const result = this.transition(types_js_1.WorkflowPhase.BLOCKED, `Blocked: ${reason}`);
        if (result.success) {
     ...
     */
    unblock(returnToPhase) {
        if (this.state.phase !== types_js_1.WorkflowPhase.BLOCKED) {
            return {
                success: false,
     ...
     */
    enterErrorRecovery(error) {
        this.state.error = error;
        const result = this.transition(types_js_1.WorkflowPhase.ERROR_RECOVERY, `Error: ${error}`);
        if (result.success) {
     ...
     * Recover from error
     */
    recoverFromError(returnToPhase) {
        if (this.state.phase !== types_js_1.WorkflowPhase.ERROR_RECOVERY) {
            return {
                success: false,
    ...
    reset(reason) {
        ...
        this.state = (0, types_js_1.createInitialState)(sessionId);
        this.state.stateHistory.push({
            phase: types_js_1.WorkflowPhase.IDLE,
            timestamp: new Date(),
            reason: `Reset: ${reason}`,
        });
    }
    ...
    static fromJSON(json, sessionId) {
        try {
     ...
     */
    transitionByAction(action) {
        const actionToPhase = {
            start_task: types_js_1.WorkflowPhase.VALUE_GATE,
            skip_to_execute: types_js_1.WorkflowPhase.EXECUTING,
            cancel: types_js_1.WorkflowPhase.IDLE,
            pass_value_gate: types_js_1.WorkflowPhase.GOAL_DEFINITION,
            define_goal: types_js_1.WorkflowPhase.CONTEXT_COLLECTION,
            collect_context: types_js_1.WorkflowPhase.ENVIRONMENT_PRIMING,
            prime_environment: types_js_1.WorkflowPhase.PLANNING,
            create_plan: types_js_1.WorkflowPhase.EXECUTING,
            start_step: null, // Handled separately
            validate_step: types_js_1.WorkflowPhase.VALIDATING,
            complete_step: types_js_1.WorkflowPhase.EXECUTING,
            drift_check: types_js_1.WorkflowPhase.DRIFT_CHECK,
            memory_check: types_js_1.WorkflowPhase.MEMORY_CHECK,
            complete_task: types_js_1.WorkflowPhase.COMPLETING,
            report_error: types_js_1.WorkflowPhase.ERROR_RECOVERY,
            recover: types_js_1.WorkflowPhase.EXECUTING,
            block: types_js_1.WorkflowPhase.BLOCKED,
